function MapAssembler(mapManager, mapData, GL){
	...
	this.assembleAngledWalls(mapData, GL);
...
MapAssembler.prototype.assembleAngledWalls = function(mapData, GL){
	var mapM = this;
	var oBlocks = [];
	var blocksInd = [];
	for (var y=0,len=mapData.map.length;y<len;y++){
	...
	for (var i=0;i<oBlocks.length;i++){
		var block3D = ObjectFactory.assembleObject(oBlocks[i], "A", GL);
		block3D.texInd = oBlocks[i].tile;
		block3D.type = "A";
		mapM.mapManager.mapToDraw.push(block3D);
	}
